...
// Алиас для /bookings/client - возвращает бронирования текущего пользователя
export async function GET(request: NextRequest) {
  try {
    ...
    if (user.role === UserRole.CLIENT) {
      ...
      // Доктор видит свои бронирования
      const bookings = await bookingService.findByDoctorId(user.id);
      return createSuccessResponse(bookings, 200, request);
